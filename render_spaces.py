...
for ckpt in ckpts:
    ...
    save_dir = f'/data/best_1023/mvsnerf_head_eca_drop/results/{ckpt_path}/nerf_space_{dir_ckpt}'
    ...
    cmd = f'--datadir /dataset/spaces_dataset  \
        --dataset_name spaces  \
        --net_type v0 --ckpt /data/best_1023/mvsnerf_head_eca_drop/{ckpt_path}/{ckpt}'  # 这里需要修改
    args = config_parser(cmd.split())
    dataset_val = dataset_dict[args.dataset_name](dataset_path=args.datadir, is_val=True, layout='small_4_1',
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_dataset):
            ...
            volume_feature, img_feat, depth_values = MVSNet(imgs[:, :3], proj_mats[:, :3], near_fars[0],
                                                            pad=args.pad)
