...
def pixel_pos_to_world_pos(pixel_x, pixel_y, z_dist, intrin, wfc):
    ...
    ref_cam_x = z_dist * (pixel_x - cx) / fx  # 第一帧原图像所在坐标系得三维X
    ref_cam_y = z_dist * (pixel_y - cy) / fy  # 第一帧原图像所在坐标系得三维Y
    ref_cam_center = torch.tensor([ref_cam_x, ref_cam_y, z_dist, 1.0], device=wfc.device)  # 以采样得中间深度当做Z（齐次坐标）
    # 得到世界坐标系上得点
...
def clamped_world_to_pixel(pos_world, cfw, intrin, crop_x, crop_y, img_w, img_h):
    ...
    in_pixel_xy_np = torch.floor(
        torch.cat([in_pixel_x.unsqueeze(-1), in_pixel_y.unsqueeze(-1)], dim=1)).cpu().numpy()  # [cams, 2]
...
def crop_scale_things(res, ref_pos, im_w_max, im_h_max, image_base_width, image_base_height):
    ...
    for idx, intrin in enumerate(res['in_intrin']):
        ...
        cropped_intrin = crop_intrinsics(base_intrin, im_h_max, im_w_max, in_pixel_xy_np[idx, 0],
                                         in_pixel_xy_np[idx, 1])
        cropped_intrin_inv = torch.inverse(cropped_intrin)
        proj_mat[:3, :4] = torch.matmul(cropped_intrin, res['in_cfw'][idx][:3, :4])
        if idx == 0:
            ref_proj_inv = torch.inverse(proj_mat)
        else:
            proj_mats += [proj_mat @ proj_mat]
        new_in_intrin.append(cropped_intrin.unsqueeze(0))
        new_in_intrin_inv.append(cropped_intrin_inv.unsqueeze(0))

    res['in_intrin'] = torch.cat(new_in_intrin, dim=0)
    res['in_intrin_inv'] = torch.cat(new_in_intrin_inv, dim=0)
    res['proj_mats'] = torch.stack(proj_mats)[:, :3]
    # adjust ref intrinsics
    ref_intrin_ext = extract_intrinsics_matrix(res['ref_intrin'])
    ref_intrin = crop_intrinsics(
        ref_intrin_ext, im_h_max, im_w_max, ref_pixel_x, ref_pixel_y)
    ref_intrin_inv = torch.inverse(ref_intrin)
    res['ref_intrin'] = ref_intrin
    res['ref_intrin_inv'] = ref_intrin_inv
    # crop image
    new_in_img = []
    ...
    tgt_intrin = crop_intrinsics(
        base_tgt_intrin, im_h_max, im_w_max, tgt_cam_xy[0], tgt_cam_xy[1]).unsqueeze(0)
    tgt_intrin_inv = torch.inverse(tgt_intrin)
    res['tgt_intrin'] = tgt_intrin
    res['tgt_intrin_inv'] = tgt_intrin_inv
    # adjust tgt image
