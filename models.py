...
class Renderer_ours(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, input_ch_feat=8, skips=[4], use_viewdirs=False):
        ...
        self.mul_head_att = MultiHeadAttention(4, 128, 8, 8)
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W)
            self.drop_feat = nn.Linear(W, W)
            self.dropout = nn.Dropout(0.05)
            self.alpha_linear = nn.Sequential(nn.Linear(W, W),
                                               nn.ReLU(),
                                               nn.Linear(W, W),
                                               nn.ReLU(),
                                               nn.Linear(W, 1),
            ...
            self.view_linear = nn.Linear(33, 128)
        else:
        ...
        self.mul_head_att.apply(weights_init)
        self.view_linear.apply(weights_init)
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            ...
            input_views = self.view_linear(input_views)
            h,_ = self.mul_head_att(bias, input_views, feature)
            del bias
            h = self.drop_feat(h)
            if self.training:
                h = self.dropout(h)
